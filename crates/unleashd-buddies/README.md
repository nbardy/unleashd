...
| Events | `appendEvent(actor, {op, payload, key?})`: mutations only, idempotent per (actor, workspace, key)<br>Every core mutation records its own event<br>`listEvents(buddyId, beforeSeq, limit)` |
| Identity | `listWorkspaces`, `getBuddy`, `listBuddies(workspaceId)`, `bindConversation` |
