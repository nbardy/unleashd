...
test('launch snapshots persist privately per request and reject another Buddy source', async () => {
  ...
  // The launching chat runs on Claude; the Buddy profile default is Codex.
  const launchingConfig = {
  ...
  try {
    ...
    // Callbacks wake on the launching harness, never the profile default provider.
    assert.deepEqual(
      (await disk.getByConversationId(first.returnConversationId))!.config,
      launchingConfig
    );
    ...
    // Re-pointing the launching chat to another harness yields a fresh return
    // thread on that harness; the earlier thread and its config are untouched.
    const repointed = {
      ...launchingConfig,
      provider: 'codex' as const,
      model: { mode: 'default' as const },
    };
    (source as unknown as { config: unknown }).config = repointed;
    const third = (await prepare(context, 'human'))!;
    assert.notEqual(third.returnConversationId, first.returnConversationId);
    assert.deepEqual(
      (await disk.getByConversationId(third.returnConversationId))!.config,
      repointed
    );
    assert.deepEqual(
      (await disk.getByConversationId(first.returnConversationId))!.config,
