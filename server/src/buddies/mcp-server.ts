...
const TOOL_DESCRIPTIONS: Record<(typeof TOOL_NAMES)[number], string> = {
  ...
  'buddy.stop':
    'Choose runId to stop one attempt, or rootMessageId to stop the entire originating work chain, including sibling requests sharing that root. Inspect get_team_state for the selected root before stopping a chain. Cancellation fences operations immediately and drains providers before releasing execution slots.',
  'buddy.retry_run':
  ...
  'buddy.get_team_state':
    'Read coordination metadata (limit 1–50, default 20): own branches, requested root descendants and supervised work. view:summary omits checkpoint/delivery detail; expand by runId with view:full. states filters before pagination. Timing measures durable queue/claim intervals; provider usage and cost are explicitly unavailable. Private chats, bodies and memory are excluded. Includes sourced project state, delivery history, effective limits and recovery controllers. Paginate with nextOffset.',
  'buddy.append_task_comment':
  ...
  'buddy.get_list':
    'Read one mailing list newest-first (limit 1–50, default 20): top-level posts, each with replyCount and latestReplyAt. A plain read (no cursor or anchor) marks the list read, thread replies included. Page from any post with one anchor: before (older posts), after (newer posts) or around (a window centred on it; a reply centres on its thread root). Every anchored page returns older/newer post ids to pass as the next before/after (null when there is no more) and moves no read mark. nextCursor still pages back from a plain read. Pass threadId to read one whole thread oldest-first; get_thread pages long threads.',
  'buddy.search_posts':
    'Keyword-search every mailing list (channel) in this workspace, thread replies included, newest first (limit 1–50, default 20). Every query term must appear (case-insensitive). mentions keeps posts that @mention a Buddy: "me" for yourself or a Buddy id; it works without a query. Narrow with listId, author ({kind:"owner"} or {kind:"buddy",buddyId}) or since (ISO time). Each match is a snippet with postId, listName and threadRootId (null for a top-level post); open the full discussion with get_thread({postId}). Follow nextCursor for older matches. Does not mark anything read.',
  'buddy.get_thread':
    'Expand the thread containing any post: pass the postId of a thread root or of a reply (such as a search_posts match). Returns the root and up to limit replies oldest-first (1–200, default 50): from the first reply for a root, or centred on the reply you passed. focusPostId names that post. older/newer are reply ids to pass as before/after (with the same postId) to page on; null means no more that way. Does not mark anything read.',
  'buddy.hire_direct_report':
        ...
        try {
          ...
          if (operation === 'buddy.send') {
            ...
            if (operation === 'buddy.get_current_work' && options.publicContract !== 'legacy') {
              ...
              result = {
                ok: true,
                data: workPage(envelope.data, input, envelope.workSnapshot),
