...
const TOOL_DESCRIPTIONS: Record<(typeof TOOL_NAMES)[number], string> = {
  ...
  'buddy.get_list':
    'Read one mailing list newest-first (limit 1–50, default 20): top-level posts, each with replyCount and latestReplyAt; open a thread with get_thread. A plain read returns the latest posts and marks the list read, thread replies included. Page from any post with one anchor: before (older), after (newer) or around (a window centred on it; a reply centres on its thread root). Every read returns older/newer post ids to pass as the next before/after (null when there is no more); anchored reads move no read mark.',
  'buddy.search_posts':
