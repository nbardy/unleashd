...
Read working and long-term memory with doc_read (kind working / long_term), even if no changes appear necessary. Compare the completed turn with existing knowledge. Use doc_read with kind note and a query to inspect relevant corrections and decisions before repeating them; when the Buddy says it already saved a lesson, look for and reuse that note. Search using one literal substring.
...
- Tasks and runs own current status, staffing, blockers, next actions and execution limits. Remove this bookkeeping from compact memory; keep any useful historical rationale in notes and an evidence pointer.
...
Use doc_write for complete replacements with kind, content, reason and the revision you read as baseRevision. On a revision_conflict, re-read, reconcile and retry; never overwrite concurrent changes with an old draft. A note (kind note) is appended.
...
export function createMemoryReviewer(options: {
  ...
  async function review(id: string, turn: CompletedBuddyTurn, signal: AbortSignal): Promise<void> {
    ...
    const writes = { working: 0, longTerm: 0, notes: 0 };
      ...
      for (const choice of MEMORY_REVIEW_MODELS) {
        ...
        const grant = grants.issueBuddy({
          ...
          observe: (tool, input) => {
            ...
            if (tool === 'doc_write')
              writes[
                kind === 'working' ? 'working' : kind === 'long_term' ? 'longTerm' : 'notes'
              ] += 1;
          },
        });
        try {
