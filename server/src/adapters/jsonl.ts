...
export async function parseCodexJsonlFile(filePath: string): Promise<CodexSession> {
  ...
  for await (const line of rl) {
    ...
    if (!isMetadata && !isRetainedEvent && !isFallbackMessage && !isBuilderOutput) continue;
    ...
    try {
      ...
      if (isCodexUserMessageEvent(entry) || isCodexAgentMessageEvent(entry)) {
        ...
        entries.push(entry);
      } else if (isRetainedEvent || isBuilderOutput) {
        entries.push(entry);
  ...
  if (!hasEventMessages) {
    entries.push(...fallbackMessageEntries);
    entries.sort(
      (a, b) =>
...
export function extractMessagesFromCodexEntries(entries: CodexSessionEntry[]): Message[] {
  ...
  for (const entry of entries) {
    ...
    if (hasEventMessages) {
      ...
      content = extractCodexContentText(entry.payload.content);
    }
    if (!content) continue;
    const timestamp = parseTimestamp(entry.timestamp) ?? new Date();
    messages.push(
