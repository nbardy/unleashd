...
async function main() {
  ...
  const p = raw.createCoordinatedProject(
    {
      ...
      buddyId: worker.id,
      title: 'Export',
      definitionOfDone: 'Saved export verified',
      todos: [{ title: 'Export', definitionOfDone: 'Export verified' }],
    },
    { actor: worker.id, key: 'p' }
  );
  const m = raw.sendCoordinatedMessage(
    {
      ...
      purpose: 'Export',
      body: 'Save export',
      execution: { mode: 'until_done', maxRuns: 1, maxDurationSeconds: 600 },
    },
    {
      policy: {
        allowed_operations: [
      ...
      returnConversationId: 'return',
    }
  );
  const child = raw.listBuddyRuns()[0];
  raw.claimBuddyRun(child.id, {
    conversationId: 'worker',
    claimToken: 'token',
    maxRuntimeSeconds: 1,
  });
  raw.startBuddyRun(child.id, 'token');
  raw.checkpointBuddyRun(child.id, {
    claimToken: 'token',
    key: 'cp',
    artifacts: [{ ref: 'saved-export.md', version: 'v1' }],
    effects: ['Saved export'],
    resume: 'Inspect saved-export.md before repeating',
    visibility: 'participants',
  });
  raw.finishBuddyRun(child.id, {
    claimToken: 'token',
    status: 'failed',
    error: 'Maximum runtime reached',
  ...
  const executor = new BuddyRunExecutor({
    ...
    createConversation: async (input) => {
      const c = {
        ...
        queue: [],
        runCoordinationMessage: async (prompt, context, token, done) => {
          received = prompt;
  ...
  console.log(
    JSON.stringify(
      {
        messageStatus: raw.getMessage(m.id).status,
        returnKinds: returns.map((r) => r.input_kind),
        checkpointSaved: raw.listRunCheckpoints(child.id).length,
        promptIncludesCheckpoint: received.includes('saved-export.md'),
        prompt: received,
      },
      null,
      2
