...
export function BuddyTeamExecutionList({
  ...
  return (
    <>
      <p>
        Observed {new Date(data.observedAt).toLocaleString()}. Request delivery, execution and
        project completion are separate records.
      </p>
      ...
      {data.items.map((run) => (
        <article className="buddy-team-execution__run" key={run.runId}>
          ...
          {run.recovery.successorRunId && <p>Recovery successor: {run.recovery.successorRunId}</p>}
