  ...
  const source = useCallback(
    async (signal: AbortSignal) =>
      TeamSetupResultSchema.parse(
        await buddyApi(
          `/api/buddies/team-configuration?workspaceId=${encodeURIComponent(workspaceId ?? '')}&key=${encodeURIComponent(result.key)}`,
          { signal }
        )
      ),
    [workspaceId, result.key]
  );
  const live = usePolledFetch(source, 5000, !!workspaceId && !!result.receipt);
  const view =
    live.data?.key === result.key && live.data.workspaceId === workspaceId ? live.data : result;
...
export function BuddyTeamConfigurationRequest({
  ...
  const previewSource = useCallback(
    (signal: AbortSignal) =>
      applied
        ...
        : messageId
          ? buddyApi(`/api/buddies/messages/${encodeURIComponent(messageId)}/team-configuration`, {
              method: 'POST',
              headers: { 'content-type': 'application/json' },
  ...
  return (
    <div className="buddy-team-configuration">
      ...
      {onEdit && (
        <button type="button" disabled={busy || previewResult.loading} onClick={onEdit}>
          {shownResult?.receipt ? 'Configure more' : 'Edit setup'}
        </button>
      )}
    </div>
  );
...
export function BuddyTeamSetup({
  ...
  const [targetId, setTargetId] = useState(initialTargetId ?? workspaceId);
      ...
      <form
        onSubmit={(event) => {
          ...
          if (mode === 'permissions') {
            ...
            const enabled = (name: string) => !revoke && form.has(name);
            ...
            const expiresAt = revoke
              ? null
              : expiry === localExpiry(grant?.expires_at)
                    ...
                    staffing: [
                      {
                        ...
                        createdBuddyIncoming:
                          enabled('staff.create') && enabled('createdBuddyIncoming'),
        ...
        <fieldset disabled={!!request}>
          ...
          {mode === 'roster' ? (
            ...
            <>
              ...
              <fieldset key={`${targetId}:${grant?.revision ?? 0}`} disabled={!permissionsEditable}>
                  ...
                  <>
                    <label>
                      <input
                        type="checkbox"
                        name="staff.create"
                        defaultChecked={grant?.capabilities.includes('staff.create')}
                      />
                    ...
                    <label>
                      <input
                        ...
                        defaultChecked={grant?.created_buddy_incoming}
                      />
                  ...
                  <>
                    ...
                    <label>
                      <input
                        ...
                        defaultChecked={grant?.capabilities.includes('relationship.write')}
                      />
                    ...
                    <label>
                      <input
                        ...
                        defaultChecked={grant?.capabilities.includes('execution.manage')}
                      />
                    ...
                    <label>
                      <input
                        ...
                        defaultChecked={grant?.capabilities.includes('schedule.manage')}
                      />
                ...
                <label>
                  ...
                  <input
                    ...
                    name="expiresAt"
                    step="any"
                    defaultValue={localExpiry(grant?.expires_at)}
                  />
                </label>
  ...
  const source = useCallback(
    async (signal: AbortSignal) =>
      BuddyTeamAccessViewSchema.parse(
        await buddyApi(
          `/api/buddies/${encodeURIComponent(buddyId)}/access/${encodeURIComponent(workspaceId)}`,
          { signal }
        )
      ),
    [buddyId, workspaceId]
  );
  const { data, error, refetch } = usePolledFetch(source, 0);
  if (error)
    return (
      <p role="alert">
        {error.message}{' '}
