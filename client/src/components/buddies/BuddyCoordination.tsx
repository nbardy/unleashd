...
export function BuddyCoordination({
  ...
  const source = useCallback(
    (signal: AbortSignal) =>
      buddyApi<CoordinationView>(`/api/buddies/${encodeURIComponent(buddyId)}/coordination`, {
        signal,
      }),
    [buddyId]
  );
    ...
    <section className="buddy-coordination" aria-label="Coordination">
            ...
            <>
                ...
                onClick={() =>
                  void change(
                    `/api/buddies/projects/${project.id}/execution`,
                    {
                      ...
                      executionState: project.execution_state === 'enabled' ? 'paused' : 'enabled',
                    },
                    'PATCH'
                  )
                }
              ...
              <button
                ...
                onClick={() =>
                  void change(
                    `/api/buddies/projects/${project.id}/execution`,
                    { key: newId(), baseRevision: project.revision, executionState: 'cancelled' },
                    'PATCH'
                  )
                }
              >
              ...
              <form
                onSubmit={(event) => {
                  ...
                  void change(
                    `/api/buddies/projects/${project.id}/execution`,
                    { key: newId(), baseRevision: project.revision, ownerId: form.get('owner') },
                    'PATCH'
                  );
                }}
              >
      ...
      >
        <fieldset disabled={saving}>
          ...
          <label>
            Every <input name="minutes" type="number" min="1" defaultValue="30" required /> minutes
          </label>
          <label>
