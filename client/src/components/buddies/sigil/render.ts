...
/** Render a Buddy genome to a PNG object URL. Browser-only (WebGL2 + canvas). */
...
/**
 * Render a workspace emblem to a PNG object URL. No strokes: the emblem is
 * soft by design, and strokes would draw hard lines across its fade.
 */
export async function renderEmblem(emblem: EmblemGenome): Promise<string> {
  ...
  gl.drawArrays(gl.TRIANGLES, 0, 3);
  return URL.createObjectURL(await canvasBlob(downsample(gl).canvas));
}
